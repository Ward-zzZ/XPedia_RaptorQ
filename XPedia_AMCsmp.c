...
*/
{
    ...
    int SRCcol = iCol*2;    int SRCfrom = iFrom*2;
    int DSTcol = PickN*2;   int DSTfrom = 0;           int MATrow = iRow;    int COLcpy = DSTcol;   int CloneN = 1;
...
*/
{
    ...
    int SRCcol = iCol;    int SRCfrom = iFrom;
    int DSTcol = PickN;   int DSTfrom = 0;           int MATrow = iRow;    int COLcpy = DSTcol;   int CloneN = 1;
...
*/
{
    ...
    MEMCHK(vfCxHs_part, MATnum*RxM*LyrN*2)
    int isPULL     = 0; // push partial H into  full H
    int isTranpose = 0;
    int iRbias     = 0;
    int iCbias     = LyrBias;
...
*/
{
    ...
    retERR_LEZ5(MPClen,NetTBS,QamNUM,BSnum,PMslen)
    int  iBUFFlen_PMs = PMslen*(MAX_MEAS_DLY+10);    float *pfGmem;   int jjj,cnt;   stAMC *psAMC;  int *pMPCmem;

    MEMCHK(viMPCs_list, BSnum*MPClen) INT_ZERO(viMPCs_list, BSnum*MPClen);

    _NEW_INIT()  _NEW_FLT0(vfCxPM_SBD, PMslen)   //MIMO_precoderRST( vfCxPM_SBD,  SubBN, TXnum,  LyrN);
...
*/
{
    ...
    ret0ERR_LEZ3(MPClen,MaxTBS,BSnum)    ret0ERR_LT(MPClen, (sizeof(stAMC)>>2))  ret0ERR_RNG(BSnum,1,3)  ret0ERR_RNG(iSNRdlyTTI,0,MAX_MEAS_DLY)   ret0ERR_RNG(iPMIdlyTTI,0,MAX_MEAS_DLY)
    int jjj,cnt;   int *pMPCmem, *piCWBITs;  float fEsN0_meas;  int iNACK;   stAMC *psAMC;  float *pfCxPMsrc, *pfCxPMuse,*pfCxPMout;  float fScaling;
    GET_GPARAM_INT(gMIMO_RandomPrecoding_ON)

    MEMCHK(viMPCs_list, BSnum*MPClen)        MEMCHK(viINFOs_list,BSnum*MaxTBS)
    MEMCHK(viCRC_list,  BSnum)               MEMCHK(vfSNR_list,  BSnum)           MEMCHK(vfCxPMsmeas, BSnum*PMslen)

    _NEW_INIT()    _NEW_FLT0(vfCxPMsnew,  PMslen)

    int iLyrPerBS = MAXOP((LyrN/BSnum),1);   if( iLyrPerBS*BSnum < LyrN ) {   iLyrPerBS +=1; }
    int SumN  = 0;  int iLyrN0;
    int BSuse = 0;
    //
    _LOOPUP_LT(cnt, 0, BSnum) {
        pfCxPMsrc  =  vfCxPMsmeas  + cnt*PMslen;   pfCxPMuse = pfCxPMsrc;
      ...
      iNACK      =  viCRCblock[cnt];
        ...
        *pMPCmem++ = iLyrN0;
        *pMPCmem++ = SumN;      SumN += iLyrN0;
        psAMC      = (stAMC *)pMPCmem;
        if( 0 == iLyrN0 ) { AMC_rst((int*)psAMC);  continue; }
        ...
        if( 0 == isPMok) { FLT_ZERO(pfCxPMsrc, PMslen);   MIMO_precoderRST(pfCxPMsrc,  SubBN, TxN, iLyrN0, 0);  }  // isRandomPM = 0



        if( gMIMO_RandomPrecoding_ON ) {  pfCxPMuse = vfCxPMsnew;     MIMO_precoderRST(pfCxPMuse,  SubBN, TxN,  iLyrN0, 1); }  // isRandomPM = 1
        ...
        if( 1 < iLyrN0 ) { fScaling = 1.0/sqrt(iLyrN0);   VEC_mulC(pfCxPMuse, SubBN*TxN*iLyrN0*2, fScaling); }   // does it work correctly ??? , added by Duan Weiming in 2021-05-21

        stRingBUF_push((int*)&psAMC->m_RingBUF_PMs, pfCxPMuse, PMslen);
        if( isPMok ) { stRingBUF_pop((int*)&psAMC->m_RingBUF_PMs, pfCxPMout, PMslen, iPMIdlyTTI, 1); } // 1 : isBlkITEM
        else         { FLT_CP(pfCxPMuse, pfCxPMout, PMslen);  }                                        // added for bug fix , by Duan Weiming in 2021-03-19
        BSuse++;

    }

    pMPCmem=viMPCs_list + 0*MPClen;  MEM_5INT(pMPCmem, RvID0, RVlen0, NetTBS0, QamType0, iMCS0)
    pMPCmem=viMPCs_list + 1*MPClen;  MEM_5INT(pMPCmem, RvID1, RVlen1, NetTBS1, QamType1, iMCS1)
