...
*/
{
    ...
    if( ( 15 <= RBnum  ) && ( 30 > RBnum ) )   { pfSRC =  vfMCSsnr_2R20RB; }
    if( ( 30 <= RBnum  ) && ( 100 >= RBnum ) ) { pfSRC =  vfMCSsnr_2R40RB; }
...
*/
{
    ...
    int VEClen = 200; int iMaxCurve = 100;  int iPlotOBJ = 0;          //float fBLERobj = 0.2;
    int jjj,lll,ilen,iSumLEN,iFind; float *pfVx,*pfVy;
    _NEW_INIT()
    ...
    iSumLEN = 0; int RealMCS_N = MINOP(MCSnum,MAX_MCS_N);  int iMcs;
...
*/
{
    ret0ERR_LT(MaxLEN, MAX_MCS_N)  MEMCHK3(viNetTBS_list,viQamType_list,vfCodeR_list,MaxLEN)  ret0ERR_RNG(RBnum,1,110)
    int SUMsc = 144*RBnum;     int jjj,Itbs, NetTBS,iQamB,iQamType;  float fCodeR;
...
*/
{
    stAMC *pstAMC = (stAMC *)pAMCmem;    ERR_LT(pstAMC->m_maxTBS,6144)
    //int RBnum = pstAMC->m_QamN/(144);    ERR_NEQ(pstAMC->m_QamN, RBnum*144)    ERR_CHKrng(RBnum,5,110)
    int  RBnum = pstAMC->m_RBuse;  ERR_CHKrng(RBnum,5,110)
    return RBnum;
}
...
*/
{
    stAMC *pstAMC = (stAMC *)pAMCmem;  retERR_RNG(NetTBS,MAX_NetTBS_MIN,MAX_NetTBS_MAX)      retERR_LIST(iRXnum,1,2,4)
    int RBnum = SUMsc/144;            ERR_NEQ(SUMsc, RBnum*144)    ERR_CHKrng(RBnum,5,110)    retERR_LIST(iModuType,2,4,16,64,256)
    ...
    int jjj;  int iITBSmin = 0;
    _LOOPUP_LT(jjj,0, MAX_ITBS_N) {  if( iminTBS <= vTBS_list[jjj] ) { iITBSmin = jjj;  break; }   }

    int *vQamB_list = gviMCS_lktbl_1lyr + MAX_MCS_N;      int iQamB = 2;
    int *vITBS_list = gviMCS_lktbl_1lyr + MAX_MCS_N*2;    int iMCSmin = 0;
...
*/
{
    stAMC *pstAMC = (stAMC *)pAMCmem;    float fEsN0_use;    int iMCS;  retERR_RNG(iLyrN,1,4)
    int RBnum = AMC_chk(pAMCmem);        int RvID = pstAMC->m_RvID;
    int iRXnum = pstAMC->m_RXnum;         retERR_LIST(iRXnum,1,2,4)
    int Transtimes = pstAMC->m_Transtimes;
    ...
    stRingBUF_pop( (int *)&pstAMC->m_RingBUFmeas, &fEsN0_use,  1 ,iDly);    fEsN0_use += fSNR_bias;  // modified by Duan Weiming in 2021-03-18
    // if( ( 0 < RvID ) && (0 < pstAMC->m_netTBS) ) { return;  }  // not update MCS if re-trans
    if( ( 0 < Transtimes ) && (0 < pstAMC->m_netTBS) ) { return;  }  // not update MCS if not new TB
    ...
    int RBuse = pstAMC->m_RBuse;        retERR_RNG(RBuse, 5,110)       pstAMC->m_QamN = iLyrN*RBuse*144;    // update m_QamN, added by Duan Weiming in 2021-03-17
    ...
    ERR_CHKrng(iMCS,0,MAX_MCS_N)
    int *vQamB_list = gviMCS_lktbl_1lyr + MAX_MCS_N;                     int iQamType = MAPQamB_QamType(vQamB_list[iMCS]);
    int *vITBS_list = gviMCS_lktbl_1lyr + MAX_MCS_N*2;                   int iITBS    = vITBS_list[iMCS];
    int *vTBS_list  = gviTBS_lktbl_1lyr + (RBnum-1)*MAX_ITBS_N;
    ...
    if ( _CHK_OPT2(iQamType, 4,16) )   { ERR_LT(0.75,fCodeRate)  }        // QPSK & 16QAM cr < 0.7,
    if ( _CHK_OPT2(iQamType, 64,256) ) { ERR_LT(0.98,fCodeRate)  }        // QPSK & 16QAM cr < 0.7,
    #endif
...
*/
{
    ...
    stAMC *pstAMC = (stAMC *)pAMCmem;   ret0ERR_LTZ(MAXlen)  nzMEMCHK(viINFObits, MAXlen)    ret0ERR_RNG(iLyrN,1,4)  ret0ERR_LTZ(iDly)
    ...
    int TT = 0;
    if (pstAMC->m_AMC_ON) {  TT = AMC_updateTT(pAMCmem); }

    pstAMC->m_RvID = TT;
    if (1 == pstAMC->m_AMC_ON) {  AMC_updateMCS(pAMCmem, fEsN0_meas, iDly, iLyrN);  }

    if ((0 == TT) && viINFObits) {  RandomBits(viINFObits, pstAMC->m_netTBS); }
